## ---------------------------
##
## Script name: 100_years_dftd_densitydep.r
##
## Purpose of script: 
##
## Author: Rowan G. Durrant
## Department of Biosciences
## Swansea University, UK
##
## Date Created: October 2020
##
## Email: RowanG.Durrant@gmail.com
##
## ---------------------------
##
## Notes:
...
if (argc < 4) {
  stop("Please specify R, A, BP, B");
} else {
  R = as.numeric(argv[1]);
  A = as.numeric(argv[2]);
  BP = as.numeric(argv[3]);
  B = as.numeric(argv[4]);
}
		...
		for(m in 1:n.mother.release){
		  ...
		  if(ind.mother_release$Age[m]  >= 156){breedSuccess = 0.56}
			...
			if(breeder==1){
				...
				if(nyoung > 0){
					offspring = setNames(data.frame(matrix(ncol = 8, nrow = nyoung)), c("Sex", "XLoc", "YLoc", "Tum.Vol", "Age", "Infected", "Death", "Dispersed"))
					for(n in 1:nyoung){
...
NewPop = function(startpop, poslocs, dispersal){
	inds = setNames(data.frame(matrix(ncol = 8, nrow = 0)), c("Sex", "XLoc", "YLoc", "Tum.Vol", "Age", "Infected", "Death", "Dispersed"))
...
RunModel = function(TumGrRate, DispDist, A, B, BP, K, MaxAge, patchDims, Sex, startpop, Time, g, R){
    print(paste("Contact Distance:", A, "Dispersal Probability:", B, "Bite Probability:", BP))
    NData2 = setNames(data.frame(matrix(ncol = 7, nrow = 1)), c("TumGrowthRate", "Infect.Dist", "Diff.Dist", "DispProb", "MedianPop", "MedianInf", "Stability"))
    ...
    #Set up populations
    for(k in 1:vcount(g)){
     	indslist[[k]] = NewPop(startpop[k], poslocs[[k]])	
    }
    ...
    save(agelist, sexlist, poplist, inflist, indslist, NData2, file=paste0("100_Year_Density_Disp_",B,"_Contact_",A,"_BP_",BP,"_rep_",R,".rda"))
...
# for(R in 1:1){
#   for(A in c(0.3,0.5)){
#     for(B in c(0.004,0.007,0.01)){
#       for(BP in c(0.6,0.8)){
      RunModel(TumGrRate, DispDist, A, B, BP, K, MaxAge, patchDims, Sex, startpop, Time, g, R)
  #       }
  #     }
  #   }
  # }
